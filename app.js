...
function renderUserInfo() {
  ...
  document.getElementById('app-subtitle').textContent = `Hallo, ${name.split(' ')[0]}!`;
}
...
const PRONOUNS_RU = {
  "ich": "я", "du": "ты", "er/sie/es": "он/она/оно",
  "wir": "мы", "ihr": "вы", "sie/Sie": "они/Вы"
};
...
async function loadState() {
  ...
  try {
    const saved = localStorage.getItem('izigerman-state-v2');
  ...
  if (currentUser) {
    try {
      ...
      if (doc.exists) {
        ...
        localStorage.setItem('izigerman-state-v2', JSON.stringify(state));
      }
...
async function confirmDeleteAccount() {
  ...
  try {
    ...
    try { localStorage.removeItem('izigerman-state-v2'); localStorage.removeItem('apnsToken'); } catch (e) {}
    state = { ...DEFAULT_STATE };
...
// показываем сразу, без стены входа (это душило конверсию: 0 триалов на 63 установки у серба).
...
// Уйти с пейволла, ничего не купив (кнопка «Не сейчас»). Гость на нативе теперь
// может попасть на пейволл сам — ему нужен выход, а не «Выйти из аккаунта».
...
// TODO: ID продуктов ниже — от стора IziTurkish. Создать Store IziGerman и два
// продукта (месяц + год) СРАЗУ в боевом режиме — товары из Test Mode не переносятся.
const MONTHLY_PRODUCT_ID = 'TODO_MONTHLY_PRODUCT_ID';
const YEARLY_PRODUCT_ID  = 'TODO_YEARLY_PRODUCT_ID';
...
function showPaywall() {
  ...
  const monthlyUrl = `https://izigerman.lemonsqueezy.com/checkout/buy/${MONTHLY_PRODUCT_ID}?checkout[custom][user_id]=${currentUser?.uid || ''}`;
  const annualUrl = `https://izigerman.lemonsqueezy.com/checkout/buy/${YEARLY_PRODUCT_ID}?checkout[custom][user_id]=${currentUser?.uid || ''}`;
...
// ============================================================
// Пул урока — это все слова, а не только глаголы: существительные и прилагательные
// не спрягаются. Поэтому упражнения на спряжение выдаём только словам с таблицей
// present, а остальным — смысловые (перевод в обе стороны и набор слова).
function canConjugate(word) {
  return !!word.present && Object.keys(word.present).length > 0;
}
...
function generateLesson(verbPool = null) {
  ...
  for (let i = 0; i < EXERCISES_PER_LESSON; i++) {
    ...
    const types = canConjugate(verb)
      ? ['conjugation', 'phrase_meaning', 'word_meaning', 'translate_to_german', 'typing']
      : ['word_meaning', 'translate_to_german', 'typing'];
    ...
    if (type === 'conjugation') {
    ...
    } else if (type === 'translate_to_german') {
      // Для глагола спрашиваем спрягаемую форму, для остального слова — само слово
      const conj = canConjugate(verb);
      const correct = conj ? verb.present[pronoun] : verb.infinitive;
      const russian = conj ? `${PRONOUNS_RU[pronoun]} ${verb.translation}` : verb.translation;
      const wrongs = conj
        ? getWrongForms(verb, correct)
        : VERBS_ONLY.filter(v => v.id !== verb.id).sort(() => Math.random() - 0.5).slice(0, 3).map(v => v.infinitive);
      exercises.push({
        type: 'translate_to_german',
        russian, verb, pronoun: conj ? pronoun : null,
        correctAnswer: correct,
      ...
      exercises.push({
        type: 'typing',
        verb, pronoun: conj ? pronoun : null,
        correctAnswer: conj ? verb.present[pronoun] : verb.infinitive
      });
    }
  }
  return exercises;
...
function getWrongForms(verb, correctForm) {
  // Считаем по уникальным формам: у многих глаголов формы совпадают
  // (du heißt = er heißt = ihr heißt), поэтому дедуплицируем до проверки.
  const forms = new Set(Object.values(verb.present || {}).filter(f => f !== correctForm));
  // Мало своих форм — добираем из других глаголов, затем из слов-инфинитивов
  if (forms.size < 3) {
    for (const v of shuffle(VERBS_ONLY.filter(v => v.id !== verb.id && canConjugate(v)))) {
      for (const f of Object.values(v.present)) if (f !== correctForm) forms.add(f);
      if (forms.size >= 3) break;
    }
  }
  if (forms.size < 3) {
    for (const v of shuffle(VERBS_ONLY)) {
      if (v.infinitive !== correctForm) forms.add(v.infinitive);
      if (forms.size >= 3) break;
    }
  }
  return shuffle([...forms]).slice(0, 3);
}
...
function renderExercise() {
  ...
  if (ex.type === 'conjugation') {
    ...
    label.innerHTML = ex.pronoun
      ? 'Напечатай форму <span class="label-badge">⌨️ сложно</span>'
      : 'Напечатай слово по-немецки <span class="label-badge">⌨️ сложно</span>';
    question.textContent = ex.pronoun ? ex.verb.infinitive : ex.verb.translation;
    subtitle.textContent = ex.pronoun
      ? `${ex.pronoun}  (${PRONOUNS_RU[ex.pronoun]})  —  ${ex.verb.translation}`
      : '';
  } else {
    label.textContent = 'Переведи на немецкий';
    question.textContent = ex.russian;
    subtitle.textContent = ex.pronoun ? `${ex.verb.infinitive}  —  ${ex.verb.translation}` : '';
  }
  ...
  grid.innerHTML = `
    <div class="typing-wrap">
      <input type="text" id="typing-input" class="typing-input"
             placeholder="Введи ответ по-немецки..."
             autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">
    </div>
    <div class="greek-keyboard">
      ${GERMAN_KEYS.map(row => `
        <div class="gk-row">
          ${row.map(ch => `<button class="gk-btn" onclick="insertGermanChar('${ch}')">${ch}</button>`).join('')}
        </div>
      `).join('')}
      <div class="gk-row gk-bottom-row">
        <button class="gk-btn gk-space" onclick="insertGermanChar(' ')">·</button>
        <button class="gk-btn gk-backspace" onclick="insertGermanChar('⌫')">⌫</button>
...
function insertGermanChar(char) {
  const input = document.getElementById('typing-input');
...
// Проверка набранного ответа. У пользователя часто нет умляутов на клавиатуре,
// поэтому Strasse == Straße, schoen == schön. Пунктуация не важна.
...
function checkTypingAnswer() {
  ...
  const isCorrect = normalizeGerman(userAnswer) === normalizeGerman(ex.correctAnswer);
...
function checkAchievements(ctx = {}) {
  ...
  const conditions = {
    ...
    'anmeldung_done': state.scenariosCompleted.includes('anmeldung'),
    ...
    container.innerHTML = `
      ...
      <div class="dialogue-card">
        <div class="dialogue-greek-wrap">
          <div class="dialogue-greek">${step.de}</div>
          <button class="speak-btn-lg" data-speak="${step.de.replace(/"/g,'&quot;')}" onclick="speakGreek(this.dataset.speak)">🔊</button>
        </div>
...
function renderVerbCards(verbs) {
  ...
  container.innerHTML = verbs.map(verb => {
    const exGreek = verb.example?.greek || verb.example?.de || '';
      ...
      ${cat.phrases.map(p => `
        <div class="phrase-card">
          <div class="phrase-top">
            <div class="phrase-greek" data-speak="${p.de.replace(/"/g,'&quot;')}"
                 onclick="speakGreek(this.dataset.speak)">${p.de}</div>
            <button class="speak-btn" data-speak="${p.de.replace(/"/g,'&quot;')}"
...
function getTutorTip() {
  if (state.lessonsCompleted === 0) return 'Начни с первого урока прямо сейчас! Каждый день — это вклад в немецкий. 🇩🇪';
  if (state.streak === 0) return 'Стрик сброшен. Помни: регулярность важнее интенсивности. 10 минут в день > 2 часа раз в неделю.';
  if (state.scenariosCompleted.length === 0) return 'Попробуй сценарий «Anmeldung» или «В банке» — это практика для реальной жизни в Германии!';
  if (!state.scenariosCompleted.includes('anmeldung')) return `Пройдено ${state.scenariosCompleted.length}/${SCENARIOS.length} сценариев. Сценарий «Anmeldung» — один из самых важных: без прописки в Германии не сделать почти ничего. Пройди его!`;
  if (state.scenariosCompleted.length < SCENARIOS.length) return `Пройдено ${state.scenariosCompleted.length}/${SCENARIOS.length} сценариев. Попробуй аптеку, Finanzamt и Ausländerbehörde — реальные ситуации в Германии!`;
  return `Отлично! Все ${SCENARIOS.length} сценариев пройдены. Следующий шаг — говорить с носителями. Найди Stammtisch или языковой обмен в своём городе!`;
}
...
async function submitSupportForm() {
  ...
  try {
    const res = await fetch('https://izigerman-webhook.vercel.app/api/contact', {
      method: 'POST',
    ...
    alert('Не удалось отправить. Попробуйте позже или напишите напрямую: support@izigerman.com');
  } finally {
    btn.disabled = false;
...
function buildPostContent(type, data) {
  switch (type) {
    ...
    case 'quiz_complete': {
      ...
      return {
        emoji: '🧩',
        title: `Квиз: ${data.categoryTitle || 'Немецкий'}`,
...
const NEWS_TOPICS = [
  ...
  { id: 'football',  label: 'Футбол',      emoji: '⚽', query: 'Bundesliga Fußball Deutschland' },
  { id: 'politics',  label: 'Политика',    emoji: '🏛️', query: 'Deutschland Politik Bundestag' },
  { id: 'economy',   label: 'Экономика',   emoji: '💰', query: 'Deutschland Wirtschaft Euro' },
  { id: 'tech',      label: 'Технологии',  emoji: '💻', query: 'Technologie Deutschland' },
  { id: 'travel',    label: 'Путешествия', emoji: '✈️', query: 'Deutschland Reisen Urlaub' },
  { id: 'culture',   label: 'Культура',    emoji: '🎭', query: 'deutsche Kultur Kunst' },
  { id: 'ai',        label: 'ИИ',          emoji: '🤖', query: 'künstliche Intelligenz Deutschland' },
  { id: 'science',   label: 'Наука',       emoji: '🔬', query: 'Wissenschaft Forschung Deutschland' },
  { id: 'cinema',    label: 'Кино',        emoji: '🎬', query: 'deutscher Film Serie' },
];
...
function showNewsLoading() {
  document.getElementById('news-feed').innerHTML = `
    <div class="news-loading">
      ...
      <div>Загружаем новости на немецком...</div>
    </div>`;
}
...
async function fetchTranslation(text) {
  ...
  try {
    const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=de|ru`;
...
function showVocab(mode) {
  ...
  if (descEl) descEl.textContent = mode === 'image'
    ? 'Выбери картинку, которая соответствует немецкому слову.'
    : 'Выбери правильный перевод немецкого слова.';
...
function completeQuiz() {
  ...
  createPost('quiz_complete', {
    ...
    categoryTitle: cat?.title || 'Немецкий'
  });
...
const BLOG_ARTICLES = [
  {
    ...
    color: 'green',
    items: [
      { slug: 'nemetskiy-alfavit',            emoji: '🔤', tag: 'Основы',     title: 'Немецкий алфавит и произношение: умляуты, ß и буквосочетания' },
      { slug: 'kak-govorit-po-nemetski',      emoji: '📚', tag: 'С нуля',     title: 'Как говорить по-немецки с нуля: пошаговый план' },
      { slug: 'nemetskiy-za-30-dney',         emoji: '📅', tag: 'Челлендж',   title: 'Немецкий за 30 дней: реальный план до первого разговора' },
      { slug: 'slozhno-li-uchit-nemetskiy',   emoji: '🧠', tag: 'Методика',   title: 'Сложно ли учить немецкий: честный ответ с примерами' },
      { slug: 'nemetskie-slova-kotorye-ty-znaesh', emoji: '💡', tag: 'Мотивация', title: 'Немецкие слова, которые ты уже знаешь — и не подозревал' },
      { slug: 'nemetskiy-po-serialam',        emoji: '🎬', tag: 'Погружение', title: 'Немецкий по сериалам: что смотреть на каждом уровне' },
    ]
  },
  {
    section: '🇩🇪 Жизнь в Германии',
    color: 'green',
    items: [
      { slug: 'arenda-kvartiry-v-germanii',   emoji: '🏠', tag: 'Быт',       title: 'Аренда квартиры в Германии: Wohnungssuche от заявки до ключей' },
      { slug: 'anmeldung-propiska',           emoji: '📋', tag: 'Документы', title: 'Anmeldung: прописка в Германии шаг за шагом' },
      { slug: 'nemetskiy-v-banke',            emoji: '🏦', tag: 'Финансы',   title: 'Немецкий в банке: открываем Girokonto и получаем карту' },
      { slug: 'nemetskiy-v-apteke',           emoji: '🏥', tag: 'Срочное',   title: 'Немецкий в аптеке и у врача: как объяснить, что болит' },
      { slug: 'finanzamt-nalogi',             emoji: '🧾', tag: 'Налоги',    title: 'Finanzamt и налоги: Steuer-ID, налоговый класс, декларация' },
      { slug: 'kak-podruzhitsya-s-nemtsem',   emoji: '🤝', tag: 'Культура',  title: 'Как подружиться с немцем: культурный код и первые шаги' },
      { slug: 'nemetskiy-dlya-raboty',        emoji: '💼', tag: 'Работа',    title: 'Немецкий для работы: резюме, собеседование, офис' },
      { slug: 'nemetskiy-v-shkole',           emoji: '👨‍👩‍👧', tag: 'Семья',    title: 'Немецкий в школе и Kita: записываем ребёнка и общаемся' },
    ]
  },
  {
    section: '💬 Разговорный немецкий',
    color: 'green',
    items: [
      { slug: '50-nemetskih-fraz',            emoji: '💬', tag: 'Фразы',     title: '50 немецких фраз для жизни: банк, магазин, аптека, ресторан' },
      { slug: 'nemetskiy-v-magazine',         emoji: '🛒', tag: 'Покупки',   title: 'Немецкий в магазине: супермаркет, касса, возврат товара' },
      { slug: 'nemetskiy-v-restorane',        emoji: '🍽️', tag: 'Ресторан',  title: 'Немецкий в ресторане: заказ, счёт и чаевые' },
      { slug: 'nemetskiy-v-transporte',       emoji: '🚆', tag: 'Транспорт', title: 'Немецкий в транспорте: Deutsche Bahn, U-Bahn, билеты' },
      { slug: 'prazdniki-germanii',           emoji: '🎉', tag: 'Культура',  title: 'Праздники Германии: что говорить и как себя вести' },
    ]
  },
  {
    ...
    color: 'purple',
    items: [
      { slug: 'nemetskaya-grammatika',        emoji: '📖', tag: 'Грамматика', title: 'Немецкая грамматика для русских: основы без лишнего' },
      { slug: 'nemetskie-artikli',            emoji: '🎯', tag: 'Грамматика', title: 'der, die, das: как запомнить род немецких слов' },
...
function openBlogArticle(slug, title) {
  ...
  if (frame) frame.src = `https://izigerman.com/blog/${slug}.html?ref=cabinet`;
...
const SPEECH_LANG_CHAIN = ['de-DE', 'de', 'ru-RU'];
...
function runSpeechRecognition(SR, langIdx = 0) {
  ...
  r.onresult = (event) => {
    ...
    const ok = alts.some(t => normalizeGermanSpeech(t) === normalizeGermanSpeech(target));
    if (ok) {
...
// Распознаватель свободно пишет ß как ss, а умляуты как ae/oe/ue.
// Приводим обе стороны к одной форме: Straße == Strasse, München == Muenchen.
function normalizeGermanSpeech(text) {
